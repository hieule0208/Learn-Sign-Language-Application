...
class _OverviewState extends ConsumerState<Overview> {
  ...
  Widget build(BuildContext context) {
    final metric = ref.watch(userMetricOverview);
    return Stack(
      children: [
        Column(
        ...
        metric.when(
          data: (metric) {
            WidgetsBinding.instance.addPostFrameCallback((_) {
              ref.read(metricProvider.notifier).setState(metric);
            });

            int totalScore = metric.totalScore;
            int level =
                OverviewController().getCurrentLevelInfo(totalScore)["level"];

            int scoreNextLevel =
                OverviewController().getCurrentLevelInfo(
                  totalScore,
                )["nextLevelScore"];
            String rank =
                OverviewController().getCurrentLevelInfo(totalScore)["title"];
            return Column(
